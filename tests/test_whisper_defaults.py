...
class WhisperDefaultTests(unittest.TestCase):
    def test_config_defaults_prefer_local_engines(self):
        tmpdir = tempfile.mkdtemp(dir=os.getcwd())
        try:
            config = Config(tmpdir)
            self.assertEqual(config.get("transcription_engine"), "whisper")
            self.assertEqual(config.get("additional_processing_engine"), "ollama")
            self.assertEqual(config.get("title_generation_engine"), "ollama")
        finally:
    ...
    def test_processing_entrypoints_default_to_large_v3(self):
        ...
        self.assertEqual(
            inspect.signature(FileProcessor.process_file).parameters["additional_processing_engine"].default,
            "ollama"
        )
        self.assertEqual(
